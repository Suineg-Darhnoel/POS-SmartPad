...
\begin{frame}{The probability of a complete word sequence}
    ...
    % Switch Chain Rule to Bigram based Calculation
...
\begin{frame}{Interpolation}
    ...
    \visible<3->{\begin{block}{In case of Trigram}
        \begin{align*}
            \hat{P} (w_n\mid w_{n-2}w_{n-1}) &= \lambda_1P(w_n\mid w_{n-2}w_{n-1})\\
                                         &+ \lambda_2P(w_n\mid w_{n-1})\\
                                         &+ \lambda_3P(w_n)
        \end{align*}
    \end{block}}
...
\begin{frame}{POS-Ngram}
    ...
    \begin{enumerate}
        \item Compute $P(c_n\mid c_{n-N+1}^{n-1}) \sim P(w_n\mid w_{n-N+1}^{n-1})$ 
